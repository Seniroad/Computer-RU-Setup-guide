...
- При отключении MMCSS время DPC ``ndis`` повышается, что несет негативные последствия, но при этом пропадает поток ``mmcss`` из System, который задействует ресурсы. Также пропадает поток ``ndis``, и вся обработка уходит на DPC. 
    ...
    - Есть способ убрать поток ``ndis``, заставить MMCSS не использовать огромное количество ресурсов (свести их почти к 0) и при этом поставить приоритет  ``audiodg`` 8-9. Поставьте у Audio в Multimedia значения Priotity и PriotityScheduling ниже, чтобы при бусте потока он получал меньший приоритет.
