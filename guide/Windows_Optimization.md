...
- При отключении MMCSS время DPC у ``ndis`` повышается, что несет негативные последствия, но при этом пропадает поток ``mmcss`` из System, который задействует ресурсы. Также пропадает поток ndis, и вся обработка уходит на DPC. 
