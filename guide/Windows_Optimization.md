...
- Откройте PowerShell от администратора (не через Win+R, и ручками тоже не получится запустить) и введите
 ...
 - Если у вас наблюдаются проблемы, напишите в личные сообщения (.seniro) для поиска проблем и и их решения.
...
- Старое правило о выставлении ровной герцовки (как например ``240.000`` вместо ``240.013``) было ошибочным. При тестировании выяснилось, что на безразрывное отображение влияет в основном лимитер (RTSS), который и зависит от герцовки. Поэтому, если вы решите разогнать свой монитор, то не бойтесь неровных значений, просто запомните, что лимит кадров должен быть кратен герцовке. 

- Поставьте все нужные разрешения вручную, чтобы избежать проблем с масштабированием в дальнейшем.

    - Советую выставлять разрешения в ``DisplayID`` (это ExtentionBlock), используйте максимально доступный вам (2.0 или 2.1 например). Так вы избежите возможных проблем или исправите проблему поддержку 240+ гц мониторов. 
...
>[!WARNING]
> Этот раздел позволяет вернуть меню 10 Windows на 11.
...
- При отключении MMCSS время DPC ``ndis`` повышается, что несет негативные последствия, но при этом пропадает поток ``mmcss`` из System, который задействует ресурсы. Также пропадает поток ``ndis``, и вся обработка уходит на DPC. 
    
    - Есть способ убрать поток ``ndis``, заставить MMCSS не использовать огромное количество ресурсов (свести их почти к 0) и при этом поставить приоритет  ``audiodg`` 8-9. Поставьте у Audio в Multimedia значения Priotity и PriotityScheduling ниже, чтобы при бусте потока он получал приоритет ниже.
      
      -  О MMCSS и о способе вы можете прочитать в гайде [EXO](https://cryptpad.fr/pad/#/2/pad/view/IEAGjIbjye6yYAbYBlSE16i02rkBFgwcpF6eKzCH7to/embed/).
