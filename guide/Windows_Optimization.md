...
- Поставьте все нужные разрешения вручную, чтобы избежать проблем с масштабированием в дальнейшем.
    ...
    - Советую выставлять разрешения в ``DisplayID`` (это ExtentionBlock), используйте максимально доступный вам (2.0 или 2.1 например). Так вы избежите возможный проблем или исправите проблему поддержку 240+ гц мониторов. 
...
- При отключении MMCSS ``audiodg.exe`` перестанет получать буст приоритета, что может вызывать статтеры звука на нестабильных системах.

- Отключение значительно понизит количество событий и ресурсов, связанных с довольно частым изменением приоритета у ``audiodg`` потока и других потоков звука.

- [Исследование о службах и их потреблении](/guide/Explorations.md)

> [!IMPORTANT]
> MMCSS является исключением, он тратит довольно большое количество циклов и событий. Это было протестировано.
