...
- Выставьте в политике прерываний не одно ядро, а все ядра, кроме 0 (потому что на 0 ядре выполняется больше всего операций).

>[!WARNING]
>Если вы используете HT(SMT), то не ставьте прерывания на разные ядра, так как при HT данный способ не работает и вся обработка уйдет на 0 ядро (0 поток). В данном случае лучше поставить прерывания на один поток, или на два потока одного ядра (Требуется дополнительное тестирование для потверждения информации *"что лучше: один поток и прерывания, прерывающие сами себя, или два потока одного ядра, но конкурирующие между собой по скорости?"*)
