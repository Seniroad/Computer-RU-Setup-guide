...
|**Шестнадцатеричное**|**Десятичное**|**Двоичное**|**Длина**|**Интервал**|**PsPrioSep**|**ПереднийКв**|**ЗаднийКв**|**ОбщийКв**|
|---|---|---|---|---|---|---|---|---|
...
- При отключении MMCSS время DPC ``ndis`` повышается, что несет негативные последствия, но при этом пропадает поток ``mmcss`` из System, который задействует ресурсы. Также пропадает поток ``ndis``, и вся обработка уходит на DPC. 
    ...
    - Есть способ убрать поток ``ndis``, заставить MMCSS не использовать огромное количество ресурсов (свести их почти к 0) и при этом поставить приоритет  ``audiodg`` 8-9. О MMCSS и о способе вы можете прочитать в гайде [EXO](https://cryptpad.fr/pad/#/2/pad/view/IEAGjIbjye6yYAbYBlSE16i02rkBFgwcpF6eKzCH7to/embed/).
