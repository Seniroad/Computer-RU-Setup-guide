...
- Данная информация уже достаточно устарела. Несмотря на то что технически масштабирование можно выполнять на GPU или с помощью скейлер-чипа монитора, кадр всё равно будет полностью обработан и передан во flip-буфер, после чего отправится на монитор уже в готовом виде.
...
>
> [!CAUTION]
> 🔒 Отключение Spectre и Meltdown может негативно сказаться на безопасности и сделать систему уязвимой. Пользователям следует тщательно оценить риски, связанные с изменением данного параметра. Данный раздел актуален только для Windows 10 и/или процессоров Intel до 9 поколения включительно.
...
- При отключении MMCSS время выполнения DPC ``ndis`` повышается, что несет негативные последствия для других потоков (потому что DPC выполняется всегда и его нельзя прервать, зато вы получаете буст в микросекундах для пинга), но при этом пропадает поток ``mmcss`` из System, который задействует ресурсы. Это происходит из-за того, что пропадает поток ``ndis``, и вся обработка уходит на DPC.
  ...
  - Есть способ убрать поток ``ndis``, заставить MMCSS не использовать огромное количество ресурсов (свести их почти к 0) и при этом поставить приоритет  ``audiodg`` 8-9. Поставьте у Audio в Multimedia значения **Priotity** ``1`` и **Scheduling Category** ``Low``, чтобы при бусте потока он получал меньший приоритет. Установите ``SystemResponsiveness`` на  ``100``, чтобы отключить MMCSS и поток ``ndis``.
...
- Просто отключите MMCSS, NetAdapterCx не регистрируется в MMCSS, поэтому можно просто отключить MMCSS поставив ``SystemResponsiveness`` на  ``100`` и значения **Priotity** ``1`` и **Scheduling Category** ``Low``, чтобы поставить приоритет ``audiodg`` 8-9.
