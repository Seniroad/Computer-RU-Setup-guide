...
Это необходимо для выполнения скриптов в репозитории. Откройте PowerShell от имени администратора и введите приведенную ниже команду.
...
>**⚠**
> Драйвера чиспета intel, которые поставляются Microsoft, имеют большой разницы с установленными вручную, как минимум на процессорах до 12 поколения, так как там уже появляются E-ядра. При желании можно вручную настроить распределение нагрузки между ядрами вручную.
...
```
[HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Control\Session Manager\kernel] "SerializeTimerExpiration"=dword:00000000
```
...
- ``disabledynamictick`` - Microsoft: *Включает и выключает функцию динамического тика таймера* ([1](https://learn.microsoft.com/en-us/windows-hardware/drivers/devtest/bcdedit--set)).
    ...
    - Для большей информации прочитайте [это](https://en.wikipedia.org/wiki/Tickless_kernel#cite_note-ars-technica-win8-1) и [это](https://arstechnica.com/information-technology/2012/10/better-on-the-inside-under-the-hood-of-windows-8/2/) 
        ...
        - Если в кратце: Отключения динамического таймера не влияет на скорость выхода из сна для приложения, запрашивающего переход в активную фазу (прерывание системного таймера, точнее на ``Sleep(n)``).

        -  **Теоретически**, выключая динамический таймер, вы запрещаете фоновым приложениям полноценно уходить в режим сна (так как теперь сон будет зависеть от приложения, а не системного таймера, ведь в прошлом разделе ([Частота прерываний по системному таймеру (разрешение таймера)](#частота-прерываний-по-системному-таймеру-разрешение-таймера)) мы упомянули, что приложения больше не зависят от него) 

- Также стоит упомянуть, что параметры в ``bcd store`` являются глобальными (то есть применяются на все разрешения таймеров)
...
> [!IMPORTANT]
> Начиная с 23h2 (Май 2023 года) фоновые процессы были ограничены в фоновом потреблении. Влияние служб на производительность нулевая. Я настоятельно рекомендую пропустить этот пункт или выполнить отключение самых неиспользуемых служб вручную, при помощи serviwin, соблюдая зависимости.
