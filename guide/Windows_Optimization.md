...
- Используйте приведенную ниже команду в качестве примера, чтобы отключить определенную настройку. Если вы оставили включенными Superfetch/Prefetch, то, скорее всего, вам нужны включенные функции, связанные с префетчингом, но не нужна компрессия в ОЗУ, которая потребляет небольшое количество циклов. (Не рекомендую даже для 16гб, потому что для новых игр нужно больше).
