...
> [Важное замечание]
> Этот гайд не рассчитан на "ультра минимальный сетап для одной игры" или "игровой сетап с сохранением полного функционала". Здесь представлено пошаговое руководство для оптимизации работы операционной системы с расчетом на выполнение задач, требующих максимальное быстродействие и отзывчивый отклик. В руководстве не запрещается свободное выполнение рекомендаций (о всех нюансах будет написано в отдельном разделе).


## Упрощение для более простого понимания 
 - **⚠** = Смайлик означает, что данное действие может привести к потере функционала или вызвать проблемы. Пожалуйста, читайте и не пытайтесь что то изменить, при выполнении пошаговой части.
 - ⛔ = Смайлик означает, что это действие может привести к отказу системы, сделать непригодной для использования или сделать хуже. Вы можете не делать данное действие. Отклонение от указанных действий не предусмотренно, как и ответственность за представленные рекомендации.**Вы делаете все на свой страх и риск.**
 - ❗= Этот смайлик работает как пометка, для информации "на подумать". В таких частях текста представлены дополнительные рекомендации, которые не вошли по определенным причинам, например недостоверности или недостатка данных, или являются размышлениями на основе некоторой информации.
  - ❓= Этот смайлик работает как пояснение для данного действия. Если у вас возникли вопросы или вам не хватает информации, попробуйте сначала прочитать этот абзац.
...
>❗Я не думаю, что драйвера чиспета intel, которые поставляются Microsoft, будут иметь разницу с установленными вручную, как минимум на процессорах до 12, так как там уже появляются E-ядра. В теории, можно вручную настроить распределение нагрузки между ядрами вручную (об этом продробнее в [1]()).
...
>❓ Отключение Microsoft Edge более целесообразная вещь, чем его удаление, ведь есть программы, которым он необходим. Данными действиями мы замораживаем работу браузера фоне, для экономии ресурсов

>❓❗ Потребление Microsoft Edge в фоновом режиме переоценено. EdgeUpdateService почти не потребляет ресурсы в фоне ([1](/docs/Edge-Service-example.png))
...
>❓ Это не означает, что автоматическое изменение скорости вентиляторов (Auto mode) перестанет работать
...
>❓❗В ходе наблюдений, выяснилось, что не все разрешения позволяют сделать ровное число в actual horizontal частоте обновления. Поэтому необходимо или вручную понижать линии отрисовки, или использовать другое количество пикселей. Моя рекомендация: в приоритете ровное число в частоте обновления.
...
- Попробуйте удалить все разрешения и прочий "мусор" (звуковые блоки), кроме родного разрешения в CRU. Это может быть решением проблемы ~1-секундного черного экрана при переключении альт-таба во время использования ``Hardware: Legacy Flip`` в режиме отображения
    ...
    >❓Здесь описывается способ проверки работы скейлинга на Дисплей, который лучше скейлинга на ГП. Это не призыв к изменению параметров в данном разделе.

    >❗ Изменения в данном разделе не несут никаких изменений в масшабировании или скейлинге. Они только ломают ключи реестра и скейлинг (Вот так работает нвидиа). Лучше никогда не трогать данный раздел и забыть про него, ведь вы уже его настроили через CRU.
...
- Откройте CMD в качестве TrustedInstaller с помощью ``C:\bin\MinSudo.exe --TrustedInstaller --Privileged`` и введите следующие команды, чтобы удалить обновления микрокода процессора
  >⛔ Вызывает BSOD на Windows 11 24h2 (23h2 с патча ххх)
...
- Отключает тикрейта-систему TSC и использует вместо него тик источника платформы (RTC)
