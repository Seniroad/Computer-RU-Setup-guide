...
>[!WARNING]
> Не отключайте MPO([1](https://github.com/Seniroad/Computer-RU-Setup-guide/blob/main/guide/Windows_Optimization.md#режимы-масштабирования)).
...
- Я не рекомендую лазить в панели и трогать разделы, кроме:
  ...
  - Управление параметрами 3D
    ...
    > Потому что остальные разделы не нужны или вредны (например настройка масштабирования, которая не является самим выбором масштабирования), когда есть аналоги, которые работают лучше (CRU).
...
- В `Управление параметрами 3D` выставьте следующие параметры:
  ...
  - Потоковая оптимизация - планирует некоторые задачи обработки, связанные с GPU, на CPU ([1](https://tweakguides.pcgamingwiki.com/NVFORCE_8.html)). Это может ухудшить частоту кадров, так как больше нагружает процессор, что критично в CPU-bound сценариях.
    ...
    - Sodium рекомендует отключать этот параметр([1](https://github.com/CaffeineMC/sodium/issues/1830)).
  ...
  - Режим низкой задержки (LLM) - Выкл
    ...
    - **Важно:** Ультра работает только для игр на DX9-DX11 (Не путать с DX11E), для DX12 обычно представлен Nvidia Reflex или он (Reflex) включен автоматически. Также стоит заметить, что не все игры/конфигурации стабильно работают с Ультра. Тестируйте, прежде чем ставить данный параметр на другое значение.
      ...
      - Вы можете просто включить ``LATENCY`` и установить ``PRERENDERLIMIT`` (``PRERENDERLIMIT`` не работает, если ``LATENCY`` имеет значение ``0x0``), чтобы убрать хард лок CPU от ``FRL_LOW_LATENCY``, получив минимальный буффер кадров.

      >Скорее всего можно включить поддержку LLM для других API.

>[!IMPORTANT]
> Неверно суждение, что работа LLM схожа с Reflex, это не так. LLM работает на уровне драйвера и синхронизирует частоту кадров со скоростью рендера кадра (исходя из документации параметров FRL_LOW_LATENCY и LATENCY) и параллельно выставляет лимит кадров до рендеринга (то есть буфер пререндера параметром PRERENDERLIMIT, который почти никогда не будет достигать размера в размере больше 1 кадра). Reflex это полноценная система, которая работает на уровне движка игры, поэтому его использование в разы эффективнее.
