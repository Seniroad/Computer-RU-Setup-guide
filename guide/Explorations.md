...
Даже самые долгие потоки svhost.exe не длятся дольше 5-6 микросекунд, что очень мало

![servicesOFF](/docs/services_work.png)

Суммарное количество потоков и их активность очень мизерная 

**Вывод**: Отключение служб без учёта системных зависимостей нарушает работу связанных компонентов. Минимальная экономия ресурсов не оправдывает потерю функциональности и стабильности системы.

**Дополнительная информация**:

- Начиная с **23h2** (Май 2023 года) фоновые процессы были ограничены в фоновом потреблении за счет снижение частоты опроса ([1](https://blogs.windows.com/windowsdeveloper/2023/05/26/delivering-delightful-performance-for-more-than-one-billion-users-worldwide)), что еще больше уменьшает фоновую работу системы.

- Данное правило не распространяется на драйвера: обработчики DPC/ISR выполняются вне контекста стандартного планировщика, а сами драйверы работают по событиям и активируются только по прерыванию. При оценке влияния также учитывайте режим исполнения: user-mode или kernel-mode.
