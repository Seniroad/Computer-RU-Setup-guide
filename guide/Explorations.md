...
Как можно заметить, разница в потреблении ```(Cycles Delta)``` минимальна. Чтобы понять разницу, можно посмотреть на потребление **Process Explorer** ```(proexp64.exe)```, который не является ресурсоёмкой программой.

**Вывод**: Проблемы от неправильного выключения служб из-за: их системные зависимостей, отказ от функций, которые привязаны к службам не стоят мизерного уменьшения потребления.
...
- На скринах можно заметить, что службы после отключения потребляют меньше памяти. На самом деле потребление службы выгружаются в файл подкачки, когда в **ОЗУ** нехватает памяти для программ. То есть, потребление служб ещё больше переоценено. 
...
На изображениях, которые были представленны выше, вы можете увидеть потребление ```explorer.exe``` с ```Обеспечить наилучшее быстродействие``` и стандартным набором.
